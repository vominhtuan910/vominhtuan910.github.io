...
```bash
sudo echo "10.129.33.153 connected.htb" >> /etc/hosts
```
...
- Port 22 (SSH): for remote access to the machine, using OpenSSH 7.4p1
...
I download the PoC code from the GitHub repository, save it as `shell.py`, and run it against the target machine.
...
After ssh successfully, we can enumerate the system to find a way to escalate our privilege to root. First I check if we can run any command as root using `sudo -l`, but it ask for password of `asterisk` user and we just log in using ssh key so we don't know the password :D
...
All three watched files are writable by `asterisk` user, so my idea is that if we can modify the command and watched files, then we can trigger the command to execute as root. I will try the first one `/etc/crontab.d/legacy` because the command has no arguments :D.
